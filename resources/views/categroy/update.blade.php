...
@section('content')
<form class="form" method="POST" action="{{ route("categroy.update") }}" enctype="multipart/form-data">
    @csrf
    <!-- Name input -->
    <div class="form-outline mb-4">
        <label class="form-label" for="categroy_name">Name :-</label>
        <input type="text" name="id" hidden value="{{$category->id}}">
        <input type="text" id="categroy_name" class="form-control" name="name" placeholder="name" value="{{ $category->name }}"
            value="{{ old('name') }}" />
        @error('name')
